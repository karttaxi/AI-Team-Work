# roguelike_shooter_fixed_v5.py
# Python pygame 기반 로그라이크 자동 슈팅 게임 수정판 v5
# 설치: pip install pygame-ce
# 실행: python roguelike_shooter_fixed_v5.py
...
def draw_start_screen(surface):
    ...
    draw_text(surface, "레벨업으로 샷건, 흡혈, 방어막 같은 능력을 얻을 수 있습니다.", SMALL_FONT, GREEN, WIDTH // 2, 435, center=True)
    pygame.display.flip()
            ...
            for enemy in enemies:
                ...
                if distance(player.x, player.y, enemy.x, enemy.y) < player.radius + enemy.radius:
                    ...
                    if damage_result == "blocked":
                        floating_texts.append(FloatingText("방어막!", player.x + 18, player.y - 34, CYAN))
