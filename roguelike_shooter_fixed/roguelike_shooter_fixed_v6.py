# roguelike_shooter_fixed_v6.py
# Python pygame 기반 로그라이크 자동 슈팅 게임 수정판 v6
# 설치: pip install pygame-ce
# 실행: python roguelike_shooter_fixed_v6.py
...
class Enemy:
    ...
    def draw(self, surface):
        ...
        if self.enemy_type == "runner":
            ...
            pygame.draw.circle(surface, WHITE, (int(self.x), int(self.y)), self.radius + 4, 3)
            ...
            for enemy in enemies:
                ...
                if distance(player.x, player.y, enemy.x, enemy.y) < player.radius + enemy.radius:
                    ...
                    if damage_result == "blocked":
                        floating_texts.append(FloatingText("BLOCK!", player.x + 18, player.y - 48, CYAN))
                        floating_texts.append(FloatingText("방어막!", player.x + 18, player.y - 28, CYAN))
